...
def style_dark(root):
    ...
    s.configure("Inst.TButton", background=FIELD, foreground=FG,
                padding=6, width=13)
    ...
    s.map("Play.TButton",
          background=[("active", "#6ce89a"), ("disabled", "#39543f")])
    ...
    s.configure("Horizontal.TProgressbar", background=ACCENT,
                troughcolor=FIELD, thickness=10)
...
class App:
    def __init__(self, root):
        ...
        self.optifine_check.pack(side="left", padx=(14, 0))
    ...
    def _sel_ev(self, _=None):
        ...
        # Load saved OptiFine preference
        ...
        def search():
            ...
            try:
                ...
                for h in hits:
                    results.insert(
                        "end",
                        f'{h["title"]}  —  {h["author"]}  ({h["downs"]}↓)'
                    )

                lbl.config(
                    text=(
                        f"{len(hits)} results for "
                        f"{self.version} / {ptype.get()}"
                        + (f" / {self.loader}" if ld else "")
                    ),
                    foreground=FG
                )
    ...
    def play(self):
        ...
        def worker():
            try:
                ...
                # Pass the checkbox state to core.
                core.launch(
                    vid,
        ...
        try:
            while True:
                ...
                if kind == "stage":
                    ...
                    self.status.config(
                        text=f"{len(data[0])} versions loaded",
                        foreground=MUTED
                    )
